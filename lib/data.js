...
export const addMovingAgerage = (data, param, size) => {
  return _.map(data, (day, index, allData) => {
    ...
    let avg = _.chain(allData)
      .slice(start, index)
      .meanBy((d) => parseInt(d[param]) || 0)
      .value()
      .toFixed(2);
...
export const generateCalculatedData = (county, countyName) => {
  ...
  results = addMovingAgerage(results, "positive_rate_percentage", 7);
  return addMovingAgerage(results, "positive_rate_percentage", 14);
};
